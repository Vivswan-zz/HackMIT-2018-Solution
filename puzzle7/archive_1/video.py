...
def hsv_correctness(QRFrame3, frame, nonQRFrame, img):
    ...
    print(g)
    return cv2.cvtColor(d, cv2.COLOR_HSV2RGB_FULL)
...
for frame in video:
    ...
    QRFrame4 = hsv_correctness(QRFrame3, frame, nonQRFrame, img)
    finalFrame = cv2.add(QRFrame4, nonQRFrame)

    # if count >= 59:
    #     finalFrame = cv2.add(QRFrame3, nonQRFrame)
    # else:
    #     finalFrame = cv2.add(QRFrame4, nonQRFrame)
    ...
    if dev_mode:
        ...
        cv2.imwrite("5_qr_3/frame%d.jpg" % count, cv2.cvtColor(cv2.add(QRFrame3, nonQRFrame), cv2.COLOR_RGB2BGR))
        cv2.imwrite("6_qr_4/frame%d.jpg" % count, cv2.cvtColor(cv2.add(QRFrame4, nonQRFrame), cv2.COLOR_RGB2BGR))
        cv2.imwrite("7_final/frame%d.jpg" % count, cv2.cvtColor(finalFrame, cv2.COLOR_RGB2BGR))
